...
if [ ! -e ../f1_beammax_clean ]; then
   echo "Could not find ../f1_beammax_clean. Make sure all four filters have been done"
   exit 
fi
...
lines=$(wc -l ../f1_beammax_clean | awk '{print $1}')
for j in $(seq 1 1 $lines); do 
    gmt gmtset COLOR_FOREGROUND=white
    file=energy_${j}_f1
    echo "File: $file"
    info=($(sed "${j}q;d" ../f1_beammax_clean))
    echo ${info[0]}
    ...
    gmt psxy ../f1_P_energy.dat -R$reg -J$proj -W1p,black -K -O  >> $file.ps
    ...
    xh_beamvesp ../f1_norm.beam ${info[0]} | awk '{print $1,$2,$4}' > vesp.dat
